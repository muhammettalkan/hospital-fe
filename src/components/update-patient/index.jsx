...
export default function UpdatePatient() {
  ...
  const getPatientById = async () => {
    const res = await patientServices.getPatientById(patientId);
    console.log("res : ", res);
    setFormData(res.data.data);
    setFormData((prevState) => {
      const { reportList, ...rest } = prevState;
      return rest;
    });
  };
  ...
  const updatePatient = async () => {
    toast.loading("Hasta bilgileri güncelleniyor...");
    // const fd = converFormData(formData);
    const res = await patientServices.updatePatient(patientId, formData);
    console.log("res : ", res);
    if (res.data.message === "Patient updated successfully") {
      toast.dismiss();
      toast.success("Hasta Bilgileti Başarıyla Güncellendi");
      setTimeout(() => {
        navigate("/patients");
      }, 1000);
    } else {
